...
fit_trunc_model <- function(
  ...
  df <- df |>
    ...
    filter(hosp_time <= trunc_time)


  stan_data <- drop_na(df, onset_to_hosp)
  ## Use the data to sample from the model posterior
  res <- stan_model$sample(
    data = list(
      n = nrow(stan_data),
      onset_to_hosp = stan_data$onset_to_hosp,
      time_since_onset = 70 - stan_data$onset_time
    )
  )
